# ALB Security Group
resource "aws_security_group" "aurouz_test_alb_sg" {
  description = "ALB Security Group"
  name        = "aurouz-test-alb-sg"
...
resource "aws_security_group_rule" "internettoalb443" {
  type              = "ingress"
  description       = "Ingress Rule on Port 443"
  security_group_id = aws_security_group.aurouz_test_alb_sg.id
  from_port         = 443
  to_port           = 443
  protocol          = "tcp"
...
resource "aws_security_group_rule" "internettoalb80" {
  type              = "ingress"
  description       = "Ingress Rule on Port 80"
  security_group_id = aws_security_group.aurouz_test_alb_sg.id
  from_port         = 80
  to_port           = 80
  protocol          = "tcp"
