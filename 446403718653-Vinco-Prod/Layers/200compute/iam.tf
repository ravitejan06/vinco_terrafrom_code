############ S3 and CW Full Access IAM User ##################
resource "aws_iam_user" "user" {
  name = "aurouz-prod-s3-cw-user"
}
    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = "arn:aws:s3:::aurouz-prod-deposit-slips/*"
      },
    ]
  })
}
...
resource "aws_iam_role_policy_attachment" "auroz_prod_AWSBackupServiceRolePolicyForBackup" {
  role       = aws_iam_role.aws_backup_role.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSBackupServiceRolePolicyForBackup"
}
resource "aws_iam_role_policy_attachment" "auroz_prod_AWSBackupServiceRolePolicyForRestores" {
  role       = aws_iam_role.aws_backup_role.name
